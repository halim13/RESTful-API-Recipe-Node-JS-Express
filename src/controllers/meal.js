...
module.exports = {
  ...
  show: async (request, response) => {
    ...
    try {
      const category = await Meal.getCategory(categoryId)
      const total = await Meal.total(category[0].id)
      ...
      const payload = await Meal.show(offset, limit, search, categoryId)
      const pageDetail = {
        total: resultTotal,
        per_page: lastPage,
        next_page: nextPage,
